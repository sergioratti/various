...
function createSweepStatus(onError, EPS) {
    ...
    var useBelow = false;
    var status = new splaytree_1.default(compareSegments);
    ...
    function compareSegments(a, b) {
        if (a === b)
            return 0;
        var ak = geom_1.getIntersectionXPoint(a, lastPointX, lastPointY);
        var bk = geom_1.getIntersectionXPoint(b, lastPointX, lastPointY);
        var res = ak - bk;
        if (Math.abs(res) >= EPS) {
    ...
    function getLeftRightPoint(p) {
        ...
        var useNext = false;
        while (current) {
            var x = geom_1.getIntersectionXPoint(current.key, p.x, p.y);
            var dx = p.x - x;
            if (dx >= 0) {
                if (dx < minX) {
                    minX = dx;
    ...
    function goOverPredecessors(root, p, res) {
        if (!root)
            return;
        var x = geom_1.getIntersectionXPoint(root.key, p.x, p.y);
        var dx = p.x - x;
        if (Math.abs(dx) < EPS) {
    ...
    function checkDuplicate() {
        ...
        status.forEach(node => {
            ...
            if (prev) {
                if (geom_1.samePoint(prev.from, current['from']) && geom_1.samePoint(prev.to, current['to'])) {
    ...
    function printStatus(prefix = '') {
        ...
        status.forEach(node => {
            var x = geom_1.getIntersectionXPoint(node.key, lastPointX, lastPointY);
            // eslint-disable-next-line
    ...
    function deleteSegments(lower, interior, sweepLinePos) {
        ...
        useBelow = false;
        if (status.size !== prevCount - interior.length - lower.length) {
...
exports.default = createSweepStatus;
//# sourceMappingURL=sweepStatus.js.map
...
const sweepStatus_1 = __importDefault(require("./sweepStatus"));
const sweepevent_1 = __importDefault(require("./sweepevent"));
/**
 * A point on a line
 *
 ...
 * @returns {ISectResult}
 */
function default_1(segments, options) {
    var results = [];
    ...
    var eventQueue = eventQueue_1.default(byY);
    var sweepStatus = sweepStatus_1.default(onError, geom_1.EPS);
    ...
    function addLowerOrInterior(s) {
        if (geom_1.samePoint(s.to, lastPoint)) {
            if (!lower)
                lower = [s];
            else
                lower.push(s);
        }
        else if (!geom_1.samePoint(s.from, lastPoint)) {
            if (!interior)
                interior = [s];
            else
                interior.push(s);
    ...
    function findNewEvent(left, right, p) {
        if (!left || !right)
            return;
        var intersection = geom_1.intersectSegments(left, right);
        if (!intersection) {
            return;
        }
        ...
        if (Math.abs(dy) < geom_1.EPS && intersection.x <= p.point.x) {
            return;
        }
        ...
        if (!current) {
            var event = new sweepevent_1.default(intersection);
    ...
    function addSegment(segment) {
        ...
        segment.angle = geom_1.angle(segment.dy, segment.dx);
        ...
        if (!isPoint) {
            if (prev) {
                ...
                var e = new sweepevent_1.default(from, segment);
                eventQueue.insert(e);
            }
            var event = new sweepevent_1.default(to);
            eventQueue.insert(event);
        }
        else {
            var event = new sweepevent_1.default(to);
...
function roundNearZero(point) {
    if (Math.abs(point.x) < geom_1.EPS)
        point.x = 0;
    if (Math.abs(point.y) < geom_1.EPS)
        point.y = 0;
}
...
//# sourceMappingURL=bentleyOttman.js.map